...
function renderizarProductos() {
  ...
  if (filtrados.length === 0) {
    grid.innerHTML = `<div class="no-resultados"><p style="font-size:32px;margin-bottom:12px">🔍</p><p style="font-size:16px;font-weight:600;margin-bottom:6px">Sin resultados</p><p style="font-size:13px">Intenta con otro término o categoría</p></div>`;
    return;
  }
  filtrados.forEach((p, i) => {
    ...
    card.innerHTML = `
      <span class="prod-cat-pill" style="background:${col.bg};color:${col.text}">${p.tipo}</span>
      <div class="prod-nombre">${p.nombre}</div>
      <div class="prod-subtitulo">${p.subtitulo}</div>
      <div class="prod-desc">${p.descripcion}</div>
      <div class="prod-footer"><span>${icono}</span><span>${p.aplicacion}</span></div>`;
...
function initBuscador() {
  ...
  if (!input) return;
  input.addEventListener('input', e => { busqueda = e.target.value; renderizarProductos(); });
}
...
function initScrollReveal() {
  ...
  document.querySelectorAll('.serv-card, .ent-card').forEach(el => {
    el.style.opacity = '0';
